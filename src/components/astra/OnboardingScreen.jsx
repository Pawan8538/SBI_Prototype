    ...
    <div className="flex flex-col items-center justify-center h-full px-8 text-center bg-white rounded-2xl shadow-sm border border-gray-200">
      ...
      <div className="grid grid-cols-2 gap-4 w-full max-w-lg mb-10 text-left">
        {PILLARS.map((p) => (
          ...
          >
            <div className={`w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center mb-1 shadow-sm`}>
              {p.icon}
            </div>
      ...
      <button
        onClick={onStart}
        className="px-8 py-3.5 bg-[#00529b] text-white font-semibold rounded-xl shadow-lg active:opacity-90 flex items-center gap-2"
        style={{
          opacity: visible ? 1 : 0,
